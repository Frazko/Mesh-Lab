...
def jar(group,name,version):
    ...
    return files[0]
compiler=jar('org.jetbrains.kotlin','kotlin-compiler-embeddable','2.4.0')
...
classpath=[android,flutter,annotations,jar('org.jetbrains.kotlin','kotlin-stdlib','2.4.0'),jar('org.jetbrains.kotlinx','kotlinx-coroutines-core-jvm','1.10.2'),jar('org.jetbrains.kotlinx','kotlinx-coroutines-android','1.10.2'),jar('junit','junit','4.13.2'),jar('org.hamcrest','hamcrest-core','1.3')]
...
subprocess.run([str(java),f'-Djava.library.path={ROOT / "target/debug"}','-cp',cp([out,*classpath]),'org.junit.runner.JUnitCore','com.frazko.mesh_host.NativeContractTest'],check=True,cwd=ROOT)
print('PASS: Kotlin 2.4.0 plugin compilation and host JNI tests; device Keystore untested')
