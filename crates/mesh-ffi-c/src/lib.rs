...
use mesh_protocol::{
    issue_authority_handoff, issue_certificate, receipt_ack_route, receipt_route, seal_message,
    verify_enrollment_request, verify_receipt_ack, AuthorityHandoff, CertificateClaims,
    DurableRecord, PolicyBundle, SealRequest, VerifiedRoster,
};
...
pub fn secure_store_receipt_ack_record(
    ...
    guarded(|| {
        ...
        let manifest = store
            ...
            .map_err(|_| Error::InternalInvariant)?;
        let ack = store
            .origin_receipt_ack(object, actor, &roster, &signer, now)
            .map_err(|_| Error::InternalInvariant)?;
...
mod tests {
    ...
    fn durable_voice_chunks_commit_before_the_receiver_exposes_audio() {
        ...
        for now in 104..124 {
            assert_eq!(
                secure_store_receipt_ack_record(owner, &[71; 32], 0, now).unwrap(),
                ack
            );
        }
        secure_store_release(owner).unwrap();
        let owner =
            secure_store_open(&[72; 32], &owner_member, owner_path.to_str().unwrap()).unwrap();
        assert_eq!(
            secure_store_receipt_ack_record(owner, &[71; 32], 0, 124).unwrap(),
            ack
        );
        let frame = RoutedRecord::decode(&ack).unwrap().frame;
        assert_eq!(
            secure_store_accept_routed(receiver, &ack, &frame.previous_hop.0, 124),
            Ok(ROUTED_ACCEPTED_RECEIPT_ACK)
        );
        assert!(secure_store_receipt_record(receiver, 0, 124)
            .unwrap()
            .is_empty());
        secure_store_release(owner).unwrap();
        secure_store_release(receiver).unwrap();
